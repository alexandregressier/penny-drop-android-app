package dev.gressier.pennydrop.adapters
...
import dev.gressier.pennydrop.databinding.PlayerSummaryListItemBinding
import dev.gressier.pennydrop.types.PlayerSummary

class PlayerSummaryAdapter :
    ListAdapter<PlayerSummary, PlayerSummaryAdapter.PlayerSummaryViewHolder>(PlayerSummaryDiffCallback()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PlayerSummaryViewHolder =
        PlayerSummaryViewHolder(DataBindingUtil.inflate(
                LayoutInflater.from(parent.context), R.layout.player_summary_list_item, parent, false
        ))

    override fun onBindViewHolder(holder: PlayerSummaryAdapter.PlayerSummaryViewHolder, position: Int) {
